...
{
    RayHit rayHit;
    mat4 viewMatrix = inverse(basicDataUBO.InverseViewMatrix);
    vec4 direction = normalize((viewMatrix*vec4(ray.Direction, 0.0)));
    ...
    for (int i = 0; i < basicDataUBO.numBounces; i++)
    {
        if (GetRayIntersection(ray, rayHit))
        {
            ...
            if (!refracted && !rayHit.FromInside && direction.x < 0) {
